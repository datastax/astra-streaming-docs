= Connect to {product_name} using Java
:page-aliases: docs@astra-streaming::astream-java-eg.adoc
...
The Java client APIs are distributed through https://search.maven.org/artifact/org.apache.pulsar/pulsar-client/{pulsar_full_version}/jar[Maven Central].
They require Java version 8. You can find the Javadocs https://pulsar.apache.org/api/client/{pulsar_full_version}/[here].
...
----
<pulsar.version>{pulsar_full_version}<pulsar.version>
----
...
* xref:operations:astream-token-gen.adoc[Managing {product_name} tokens].
