...
Pulsar's xref:astream-subscriptions-shared.adoc[shared subscription] model trades an increased message processing rate for ordering guarantees. In a round-robin delivery, there's no way for the broker to know which messages are going to which consumer.
...
You must also tell Pulsar what `keySharedPolicy` this subscription will use.
The example below uses the auto-hashing `keySharedPolicy`.
