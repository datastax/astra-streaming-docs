...
An _exclusive* subscription_ describes a basic publish-subscribe pattern where a single consumer subscribes to a single topic and consumes from it.
...
This example uses the following `pulsarConsumer` object in `SimplePulsarConsumer.java`:
...
[IMPORTANT]
====
Pulsar creates an exclusive subscription by default if you don't delcare a `subscriptionType`.
====
