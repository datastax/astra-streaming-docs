...
You can get this information with `xref:apis:index.adoc#get-tenant-information[GET 'https://api.astra.datastax.com/v2/streaming/tenants']` or from the xref:operations:create-delete-tenants.adoc#get-tenant-information[**Tenant Details** section in the {astra-ui}].
...
[source,curl]
----
curl -sS -L -X GET "$WEB_SERVICE_URL/admin/v3/sinks/builtinsinks" \
...
[source,curl]
----
curl -sS -L -X GET "https://$WEB_SERVICE_URL/admin/v2/namespaces/$TENANT" --header "Authorization: Bearer $PULSAR_TOKEN" | python3 -mjson.tool
----
