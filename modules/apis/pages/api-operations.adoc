...
* An active {astra_db} account
* An application token and Pulsar token for {product}
...
For managing {product} Namespaces, we use the native https://pulsar.apache.org/admin-rest-api/[Pulsar REST APIs].  These are documented on the Apache Pulsar Docs for REST API.
...
== {product} JWT Token DevOps APIs

=== List Existing Tokens IDs

Get a list of Token IDs for your Cluster.  With the TokenID, you can then lookup and obtain the Pulsar JWT Token string.  The TokenIDs are also listed in the {astra_ui} for that Tenant and Cluster.
...
The new JWT Token will also be visible in the {astra_ui} for that Tenant and Cluster.
