...
In a *Pay As You Go* environment, you can create tenants in any of the xref:astream-regions.adoc[supported regions], while *dedicated environments* are open to almost any public cloud region.
====
...
To integrate a custom SSO provider, you will need a non-default Astra Organization.
...
[%collapsible]
====
Unless you are starting a project from scratch, you likely have message data that needs to be brought over to your Astra Streaming tenants.
The Streaming Team has quite a bit of experience with this and can help you find the right way to migrate. mailto:streaming@datastax.com[Email the team] for more information.

Every tenant in Astra Streaming comes with custom ports for Kafka and RabbitMQ workloads.
