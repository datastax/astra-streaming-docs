...
Each externally exposed raw Astra Streaming metric is specific to a particular Pulsar server instance. The same raw metrics could come from multiple server instances. From a {product_name} user’s perspective, the direct monitoring of raw metrics is not really useful. Raw metrics need to be aggregated first - for example, averaging or summing the raw metrics over a period of time.

Prometheus provides a powerful but easy-to-use query language, PromQL, for selecting and aggregating time series data in real time. PromQL syntax is beyond this document's scope, but the https://prometheus.io/docs/prometheus/latest/querying/basics/[Prometheus documentation] is a good place to start.
...
The examples below are filtering out system messages with the pattern `{topic !~ ".*__.*"}`.
This PromQL pattern filters out messages when their topic labels do not include “__”.
This pattern assumes that the user applications don’t have namespaces and topics with “__” as part of the names.
Otherwise, they will be excluded as well.
...
----
[%header,format=csv,cols="1,3,2"]
|===
include::example$psql-queries.csv[]
|===
