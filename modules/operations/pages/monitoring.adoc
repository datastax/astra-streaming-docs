...
=== Alerting Rules general guideline
In a perfect world, these metrics should always stay at 0, but in reality, these metrics will increase when the application workload becomes heavier. If the system is behaving correctly, these metrics should go down when the application workload drops.

So one way to trigger an alert on these metrics is to simply set a threshold beyond which the alert is triggered. However, this is likely going to cause false alarms when there is an incoming workload spike.

A better approach would be to calculate the increase rate of these metrics over a period of time (e.g. 1 hour) and set a threshold on it. For example if the average message backlog increase rate is beyond a certain threshold, an alert is triggered.

The choice of the actual threshold values for these metrics will be highly dependent on each application’s workload and requirement. But they should be some relatively large positive numbers, e.g. several hundreds or several thousands. Otherwise, they may trigger too many false alarms.

== Grafana
Grafana Dashboards for Astra Streaming Metrics
A set of Grafana dashboards have been pre-built around core message processing for the exposed Astra Streaming metrics which can be found from: 
https://github.com/datastax/astra-streaming-examples/tree/master/grafana-dashboards
