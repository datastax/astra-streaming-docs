...
. xref:getting-started:astream-quick-start.adoc#produce-some-messages[Produce some messages] in your tenant with the Pulsar CLI or the {product_name} Websocket.
...
For more on {product_name}, see the xref:ROOT:astream-faq.adoc[FAQ] or the xref:getting-started:astream-quick-start.adoc[Quickstart]. +
