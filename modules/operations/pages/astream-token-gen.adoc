...
{product} uses two types of tokens that you can generate in {astra}.

[#astra-token]
== {astra} application tokens

{astra} application tokens are access tokens for authentication to the {astra-ui} and the xref:apis:index.adoc[{product} {devops-api}].
You use these tokens for tenant administration, such as creating tenants and {pulsar-short} tokens (JWT)

To create an {astra} application token, see xref:astra-db-serverless:administration:manage-application-tokens.adoc[].
...
If necessary, {product} forwards the JWT to the {pulsar-short} cluster for role verification.
...
{product} {devops-api}::
To send requests to the xref:apis:index.adoc[{product} {devops-api}], use an {astra} application token.
+
...
curl -sS --fail -L -X GET "$WEB_SERVICE_URL/admin/v3/sinks/builtinsinks" \
  --header "Authorization: Bearer $ASTRA_PULSAR_TOKEN"
----
...
+
The following example commands use a {pulsar-short} token declared in the `pulsar-admin` binary configuration so the token doesn't have to be passed directly on the command line:
+
[source,shell]
----
...
{astra-cli}::
Use an {astra} application token to authenticate with the xref:developing:astra-cli.adoc[{astra-cli}].
