...
Consider setting environment variables for sensitive and frequently used values, such as your authentication credentials and xref:operations:create-delete-tenants.adoc#get-tenant-information[tenant connection details]:
...
----
curl -sS -L -X GET "https://api.astra.datastax.com/v2/stats/namespaces/**TENANT_NAME**" \
...
[source,curl]
----
curl "$WEB_SERVICE_URL/admin/v3/sinks/builtinsinks" \
