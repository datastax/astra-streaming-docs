...
----
kubectl create secret generic **SECRET_NAME** --from-file=config.yml --dry-run=client \
  -oyaml > k8s-additional-scrape-config-demo.yaml
...
kubectl rollout status daemonset kubeprom-prometheus-node-exporter --timeout 60s
----
