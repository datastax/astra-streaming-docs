...
[IMPORTANT]
====
Change Data Capture (CDC) for {astra-db} incurs billed charges based on your {product} usage.
...
{product} processes data changes through an {pulsar-reg} topic.
...
For columns using unsupported data types, those data types are omitted from the events that CDC sends to the {product} tenant's `data-` topic.
...
To configure CDC for {astra-db}, you must create an {product} tenant, an {astra-db} database, and a table in that database.
...
You can use xref:connectors:index.adoc[other {product} sinks].

[#create-tenant]
=== Create a streaming tenant

CDC operates through {product} tenants and topics.
If you don't have any {product} tenants, you must create a tenant in order to enable CDC on a table in an {astra-db} database.
...
+
Your {product} tenant must be in the same region as the table where you want to enable CDC.
+
CDC for {astra-db} is available only in xref:operations:astream-regions.adoc[regions that support {product}].
If your database is deployed to an xref:astra-db-serverless:databases:regions.adoc[{astra-db} region] that doesn't yet support {product}, contact your {company} account representative or {support-url}[IBM Support].
...
. If you plan to enable CDC on multiple databases or <<multi-region-cdc,enable CDC on a multi-region database>>, create at least one {product} tenant for each region where your databases are deployed.
...
+
Make sure the database is deployed to the same region as your {product} tenant.

. Click *{cql-console}* to use the built-in {cql-shell} (`cqlsh`) to create a table in your database.
+
...
To enable CDC, you must deploy the database to xref:operations:astream-regions.adoc[regions that support {product}].
...
* `**STREAMING_CLUSTER_NAME**` and `**STREAMING_TENANT_NAME**`: The name of the {product} tenant and cluster.
...
* The `data-` topic consumes CDC data in {product}.
...
If you enable CDC on multiple tables in the same region, each table has its own topics within the corresponding regional {product} tenant.
...
This indicates that {product} successfully sent changes tracked by CDC to your sink service deployment.
+
...
. <<create-tenant,Create at least one {product} tenant for each region>> where you want to enable CDC.
+
If your database is deployed to a region that doesn't support {product}, contact your {company} account representative or {support-url}[IBM Support].
...
These cross-region repairs are separate from CDC.

When you enable CDC on a multi-region database, CDC emits write events when any CDC-enabled table in any CDC-enabled regions receives a write directly.
This means that your sinks could receive duplicate write events, depending on your application's write pattern across regions.

If your client applications send the same write to the same table in multiple regions, the `data-` topics in the corresponding region-specific tenants _all_ receive that write event.
As a result, those topics will pass concurrent, duplicate events for the same row to the sink.
{astra-db} doesn't reconcile these concurrent modifications to the same row in multiple regions.
...
Alternatively, design your applications to write to one region only to avoid duplicate CDC write events.
{astra-db}'s eventual consistency policy replicates the write to the other regions through cross-region repair _without_ generating duplicate CDC write events.

The appropriate strategy depends on your workload's tolerance for potential delays in cross-region repairs.

Be aware that cross-region writes can be received out of order or be missed.
Furthermore, ordering between regions isn't guaranteed.

[#get-cdc]
== Check CDC status

You can check your active CDC configurations in the {astra-ui} or with the {astra} {devops-api}.
...
=== Check CDC status with the {astra} {devops-api}
...
For multi-region databases with CDC enabled in multiple regions, the `regions` array includes the {product} configuration for each region.
+
[source,json]
----
{
...
+
The response includes the CDC status and, if available, the {product} configuration.
For multi-region databases with CDC enabled in multiple regions, the `regions` array includes the {product} configuration for each region.
+
[source,json]
----
...
. If you removed any tables from the existing CDC configuration, decide whether you want to delete the associated namespace, topics, and {product} tenant.
...
Disabling CDC doesn't remove the associated namespace, topics, or {product} tenant:

* If you reenable CDC for the same table, the existing topics are reused with the existing records.

* If you want to discard a table's CDC records, you must manually remove the associated {product} artifacts after disabling CDC:
+
** If you remove CDC from a table, then you can delete the table's `data-` and `log-` topics from the `astracdc` namespace in the {product} tenant.
...
** If you remove CDC from all tables in a region, and you no longer need CDC in that region, then you can delete the `astracdc` namespace from the {product} tenant in that region.
Deleting the namespace also deletes the topics within that namespace.
** If you remove CDC from all of a database's tables, and you no longer need an {product} tenant for any reason, you can delete the entire tenant.
...
==== Disable CDC with the {astra} {devops-api}
...
* xref:ROOT:astream-faq.adoc[{product} FAQs]
* xref:developing:clients/index.adoc[Use {pulsar-short} clients with {product}]
