= Producing and consuming messages using the Java pulsar client with {product} and Spring
:navtitle: Spring
...
. Replace the values in your Java application with values from the Connect tab of your {product} instance.
+
[source,java]
----
...
----
+
You could instead modify the values in the "application.properties" file in the /resources folder, which is a better practice for production applications. The Connect tab in {product} has a link to download a properties file with the values your application needs to connect to your {product} instance.
+
...
. Run the project and send a message to your {product} cluster:
+
[source,bash]
----
mvn spring-boot:run
...
Remember to check in your {product} instance to see the message you sent to the newly created subscription, "my-subscription".
