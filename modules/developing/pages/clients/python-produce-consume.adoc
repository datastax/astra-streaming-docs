= Use the Python Pulsar client with {product}
:navtitle: Python
:description: Use the Python Pulsar client with {product} to produce and consume messages.
:page-tag: astra-streaming,dev,develop,python,pulsar

You can use the Python Pulsar client with {product} to produce and consume messages.
...
This code creates a Pulsar client instance with the topic URL and token authentication.
+
.index.py
[source,python]
----
