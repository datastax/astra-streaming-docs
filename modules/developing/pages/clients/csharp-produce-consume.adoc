...
TIP: Visit our {astra-streaming-examples-repo}/csharp[examples repo] to see the complete source of this example.
...
It's how time! Asynchronously send a single (string) message and wait for acknowledgment.
...
Loop through the messages provided to the consumer's subscription and write its contents.
...
The moment of truth... run the app
...
[source,shell,subs="attributes+"]
----
include::{astra-streaming-examples-repo}/csharp/output.sh[]
----
