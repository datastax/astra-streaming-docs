...
. Upload function code or select a built-in {company} transform function:
+
[tabs]
======
...
[TIP]
=====
You can also deploy transform functions in sinks.

.Deploy a transform function in a sink
[%collapsible]
====
By default, functions transform data either after it is written to a topic by a source connector, or before it is read from a topic by a sink connector.
This requires either a custom connector or an intermediate topic (with additional storage, IO, and latency).

Alternatively, you can deploy transform functions in a sink to apply preprocessing to sink topic writes (outgoing messages from the sink).

You can deploy the transform function while creating the sink:

. xref:streaming-learning:pulsar-io:connectors/index.adoc[Create a sink] and include the transform function with your sink configuration.
...
.. Configure the sink and select a transform function to deploy with the sink.
...
+
For transform functions, see the xref:developing:astream-functions-transform.adoc[transform functions configuration reference].
+
For general information about function configuration, see the https://pulsar.apache.org/functions-rest-api/#operation/registerFunction[{pulsar-short} documentation].
