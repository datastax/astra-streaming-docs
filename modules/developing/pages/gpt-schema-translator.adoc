...
== JSON-to-{cql-short} mapping example

This example uses a JSON schema for a {pulsar-short} topic, and a {cql-short} schema for an {astra-db} table.
...
=== AVRO-to-{cql-short} mapping example
...
. The {astra-db} sink connector writes data to the {cass-short} table with a {cql-short} schema.
+
.{cql-short} schema example
[source,cql]
----
...
+
The {gpt-schema-translator} generates an AVRO-to-{cql-short} schema mapping while messages are processed.
+
[source,plain]
----
