...
This tool uses generative AI,based on the `GPT-4` model, to automatically generate schema mappings between {product} ({pulsar-short} topics) and the {astra_db} sink connector (Cassandra tables).
...
This example uses a JSON schema for a {pulsar-short} topic, and a CQL schema for an {astra_db} table.
The {gpt-schema-translator} generates a mapping between the two schemas that the {astra_db} sink connector can use to write data from the {pulsar-short} topic to the {astra_db} table.
...
When you create the mapping in the {astra_ui}, you can click *Generate Mapping* to create a generic {pulsar-short} topic schema based on your Cassandra table schema.
