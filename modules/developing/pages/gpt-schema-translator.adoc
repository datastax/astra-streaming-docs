...
* An xref:connectors:sinks/astra-db.adoc[{astra-db} sink connector].
...
+
The following examples show the progression of the {cass-short}-to-{pulsar-short} schema mapping:
+
{cass-short} table schema as JSON::
+
[source,json]
----
{
...
. The {astra-db} sink connector writes data to the {cass-short} table with the equivalent CQL schema:
+
[source,json]
----
...
The following examples show the same schema in CQL, CQL-as-JSON, and the generated JSON schema mapping:

{cass-short} table schema in CQL::
+
[source,cql]
----
...
{cass-short} table schema in JSON::
+
[source,json]
----
{
...
Translated {pulsar-short} JSON schema mapping::
The schema translator generates the following {pulsar-short} JSON schema mapping based on the {cass-short} table schema:
+
[source,console]
----
passportnumber=value.passportnumber, age=value.age, firstname=value.firstname, lastname=value.lastname
