...
Next, start the connectors and run the pipeline.

These steps start the processing pipelines, send a single record of data through them, and then validate that everything happened as expected.
Optionally, you can put the processing pipeline under load with a bulk of data.
...
=== Ingest a simulated REST payload

Use a simulated ingestion to test the connections and pipelines:
...
After the REST connector ingests the simulated click event data, the normalize pipeline receives the raw click data, processes it, and then passes the filtered event data to the product clicks pipeline.
Through the {company} {product} connections, the data is streamed to {astra_db} and written to your database.
The raw clicks are written to the `all_clicks` table, and the filtered clicks are written to the `product_clicks` table.

You can confirm that the data was received and processed by inspecting the metrics for the pipelines and connectors.
The following steps inspect the raw clicks data flow, and then the product clicks data flow:
