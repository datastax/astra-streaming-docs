= Build real-time data pipelines with {product} and Decodable
...
[TIP]
====
This tutorial builds complete real-time data pipelines with third-party integrations.
For a simplified tutorial that focuses on the {product} components only, see the xref:getting-started:index.adoc[quickstart].
====
...
Decodable::
This tutorial uses Decodable's real-time stream processing, which is powered by {flink-reg}.
...
{astra}::
Within {astra}, the tutorial uses {astra-db} and {product}, which are powered by {cass-reg} and {pulsar-reg} respectively.
Specifically, you will create the following {astra} objects:
+
...
* An {astra-url}/signupstreaming[{astra} account^]
...
. In the {astra-ui-link} header, click icon:grip[name="Applications"], and then select *{astra}*.
...
. From your database's overview page, click **{cql-console}**, and then wait for the built-in {cql-shell} (`cqlsh`) to start.

. Enter the following {cql-short} statement into the {cql-console}, and then press kbd:[Enter].
+
...
After you create the required {astra} objects, you can create pipelines for processing incoming data and connectors that bond a Decodable stream of data with the {product} topics.
...
* **Broker Service URL**: Enter the {pulsar-short} broker service URL from your {product} tenant.
* **Web Service URL**: Enter the {pulsar-short} web service URL from your {product} tenant.
* **Topic**: Enter `persistent://webstore-clicks/production/all-clicks`.
* **Authentication Token**: Enter the {pulsar-short} token you created for your {product} tenant.
...
After the REST connector ingests the simulated click event data, the normalize pipeline receives the raw click data, processes it, and writes it to the `all_clicks` table in {astra-db}.
Because the simulated payload was a product click, the product clicks pipeline receives the normalized click data, and then writes it to the `product_clicks` table in {astra-db}.
...
. Go to your **webstore-clicks** database, click **{cql-console}**, and then run the following {cql-short} statement to inspect the data that was written to the `all_clicks` table:
+
[source,cql]
----
...
. Go to your **webstore-clicks** database, click **{cql-console}**, and then run the following {cql-short} statement to inspect the data that was written to the `product_clicks` table:
+
[source,cql]
----
